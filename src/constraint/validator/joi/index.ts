...
export const toJoi = (attributes) => attributes?.reduce(
  (acc, { key, type, isArray, required }) => (
    { ...acc, [key]: isArray ? Joi.array() : type === 'boolean' ? Joi.boolean().default(false) : type === 'number' ? Joi.number() : type === 'location' ? Joi.object() : required ? Joi.string().required() : Joi.string().allow('') }), {}) || {}
